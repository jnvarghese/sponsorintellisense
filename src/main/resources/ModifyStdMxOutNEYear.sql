 DELIMITER //
 DROP PROCEDURE IF EXISTS ModifyStdMxOutNEYear;
 CREATE PROCEDURE ModifyStdMxOutNEYear()
   BEGIN
 ...
 DECLARE cur1_2 CURSOR FOR SELECT sm.id,maxout,expirationMonth,expirationYear FROM student_maxout sm, sponsee s WHERE sm.enrollmentid IN (SELECT id FROM enrollment WHERE STATUS=0 AND renewed='N' AND id NOT IN (120,122,125,128,131,132,133,134,749,778,797,1018,1057,1331,1341,1702,1935)) AND s.enrollmentId=sm.enrollmentId AND s.studentId NOT IN (SELECT studentid FROM student_maxout WHERE enrollmentid IN (SELECT id FROM enrollment WHERE STATUS=1 AND renewed='Y')) AND sm.studentid NOT IN (SELECT studentid FROM student_maxout WHERE enrollmentid IN (SELECT id FROM enrollment WHERE STATUS=1 AND renewed='Y')) AND DATE_FORMAT(sm.maxOut, '%m') <> s.expirationMonth AND DATE_FORMAT(sm.maxOut, '%Y') <> s.expirationYear and expirationMonth <> 0 AND (flag1=1 OR flag1=0) AND (flag2=0 OR flag2=2) GROUP BY sm.studentId ORDER BY sm.enrollmentId;
...
read_loop: LOOP
 ...
 SET str_date = STR_TO_DATE(CONCAT(my_expy,'-',my_expm,'-',my_day), "%Y-%m-%d");
  ...
  UPDATE student_maxout SET maxOut = str_date, flag3=3 WHERE id = my_sm_id;
