DELIMITER //
 DROP PROCEDURE IF EXISTS ModifyStdMxOutNEMonthAndYear;
 CREATE PROCEDURE ModifyStdMxOutNEMonthAndYear()
   BEGIN
...
read_loop: LOOP
 ...
 SET str_date = STR_TO_DATE(CONCAT(my_expy,'-',my_expm,'-',my_day), "%Y-%m-%d");
  ...
  UPDATE student_maxout SET maxOut = str_date WHERE id = my_sm_id;
...
close cur1;
 ...
 -- CALL ModifyStdMxOutNEMonthAndYear();