...
public interface SponsorMapper {
	...
	*/
	public static final String SELECT_ALL_ACTIVE_SPONSORS_BY_PARISHID = "SELECT S.ID,EN.ID ENROLLMENTID, EN.MISCAMOUNT, en.effectiveDate, "
			...
			+ "AND C.REGIONID = R.ID AND S.PARISHID = #{id} AND S.SPONSORSTATUS = 0 GROUP BY EN.ID ORDER BY SPONSORCODE, EN.ID";
...
//	/profilePicture
	@Select("SELECT CONCAT(A.CODE,'-',P.CODE,'-',ST.STUDENTCODE) UNIQUEID, STUDENTNAME,ST.ID studentId, uploadstatus, P.ID projectId, imageLinkRef,  "
			...
			+ " HOBBIES hobby, A.NAME AGENCYNAME, P.NAME PROJECTNAME, P.ADDRESS ADDRESS FROM ENROLLMENT EN, SPONSEE SPE, "
