...
public interface SponsorMapper {
	...
	public static final String SELECT_ALL_ACTIVE_SPONSORS_BY_PARISH = "SELECT S.ID,  P.NAME PARISHNAME, P.CITY PARISH,S.CITY, "
			+ "CONCAT(R.CODE,'-',C.CODE,'-',P.CODE,'-',S.SPONSORCODE) SPONSORCODE, FIRSTNAME, LASTNAME, MIDDLEINITIAL, NICKNAME,STREET, "
			+ "S.CITY, STATE,POSTALCODE, PARISHID FROM SPONSOR S, PARISH P ,CENTER C,REGION R   WHERE S.PARISHID =P.ID "
			+ "AND P.CENTERID = C.ID AND C.REGIONID = R.ID AND S.SPONSORSTATUS = 0 AND P.ID= #{id};";
	...
	*/
	public static final String SELECT_ALL_ACTIVE_SPONSORS_BY_PARISHID = "SELECT S.ID,(CASE WHEN EN.ID IS NULL THEN RAND() ELSE EN.ID END) ENTID, EN.MISCAMOUNT, "
			...
			+ "FROM SPONSOR S LEFT JOIN ENROLLMENT EN ON  EN.SPONSORID= S.ID AND EN.STATUS =0 LEFT JOIN SPONSEE SE ON EN.ID=SE.ENROLLMENTID AND SE.STATUS=0, "
	...
	@Select(SELECT_ALL_ACTIVE_SPONSORS_BY_PARISH)
	List<Sponsor> listByParishId(@Param("id") Long id);
