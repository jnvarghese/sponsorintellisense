...
public interface StudentMapper {
	...
	@Select("SELECT S.ID, CONCAT(A.CODE,'-',P.CODE,'-',S.STUDENTCODE) STUDENTCODE, S.STUDENTNAME, S.GENDER, S.GRADE, "
			+" OCCUPATIONOFGUARDIAN,BASELANGUAGE,HOBBIES,TALENT FROM STUDENT S ,PARISH_PROJECT PP , PROJECT P, AGENCY A" + 
			" WHERE S.PROJECTID = PP.PROJECTID AND PP.PROJECTID = P.ID AND P.AGENCYID = A.ID " + 
			" AND PP.PARISHID = #{parishId} AND PP.PROJECTID = #{projectId} AND GENDER=#{gender} AND S.STATUS = 0 ORDER BY S.STUDENTNAME")
	List<Student> listStudentsByParishAndProjectAndGender(
			@Param("parishId") Long parishId,
			@Param("projectId") Long projectId, @Param("gender") String gender
			);
	...
	@Select("SELECT CONCAT(A.CODE,'-',P.CODE,'-',ST.STUDENTCODE) UNIQUEID, STUDENTNAME,ST.ID studentId, uploadstatus, P.ID projectId, imageLinkRef,"
			...
			+ " AND EN.ID = #{id}  AND SPE.STATUS=0 ORDER BY SMAX.maxOut ")
	...
	@Select("SELECT E.ID ENROLLMENTID, E.EFFECTIVEDATE,E.ACTUALAMOUNT,E.CONTRIBUTIONAMOUNT,E.MISCAMOUNT, "
			...
			+ "ST.STUDENTNAME, CONCAT(A.CODE,'-',P.CODE,'-',STUDENTCODE) STUDENTCODE, ST.GENDER, ST.GRADE, ST.occupationOfGuardian, ST.nameOfGuardian, p.NAME projectName"
			+ " FROM ENROLLMENT E, SPONSEE SP, STUDENT_MAXOUT SM , STUDENT ST, PROJECT P, AGENCY A, USERS U "
			+ "WHERE ST.PROJECTID= P.ID AND P.AGENCYID= A.ID AND  E.ID=SP.ENROLLMENTID AND E.ID = SM.ENROLLMENTID AND SP.STUDENTID=ST.ID AND SP.STUDENTID = SM.STUDENTID "
